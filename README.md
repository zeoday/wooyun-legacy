...
**WooYun Legacy** 是一个 [Claude Code](https://docs.anthropic.com/en/docs/claude-code) 插件，基于 WooYun（2010-2016）收录的 22,132 个业务逻辑漏洞案例，提炼出覆盖 6 大领域、33 类漏洞的 Web 应用安全测试方法论。

安装后，Claude 不再是泛泛而谈的安全顾问——它能引用真实公司案例、给出量化统计、按 WooYun 独有的分类法输出精准的测试方案。
...
| 维度 | 无插件的 Claude | 装了 WooYun Legacy |
|------|----------------|-------------------|
| 案例引用 | 通用安全建议，无真实案例 | 引用具体公司名 + 漏洞描述（如 "M1905 电影网 2588 元套餐只要 5 毛"） |
| 数据支撑 | 主观判断（"风险较高"） | 量化统计（"支付绕过 1,056 案例，68.7% 高危"） |
| 攻击分类 | 常规 OWASP 分类 | WooYun 独有「任意X」分类法（任意账号/操作/查看/修改/删除） |
| 测试深度 | 3-5 种攻击模式 | 每个领域 10+ 种攻击模式 + 绕过技巧 |
| 方法论 | 列出检查项 | 四阶段结构化方法（映射→假设→测试→报告）|
...
| 场景 | 示例提问 |
|------|---------|
| 密码重置安全 | "帮我测试密码重置功能的安全性" |
| 弱口令检测 | "对这批政府系统做弱口令检测" |
| 验证码绕过 | "测试这个金融 App 的验证码安全" |
| 信息泄露排查 | "审计这个 Spring Boot 系统的信息泄露" |
| 配置安全加固 | "检查 Tomcat + MySQL + Redis + Nginx 的配置安全" |
| 云环境安全 | "制定阿里云/AWS 环境安全检查方案" |
| 银行 App 评估 | "对手机银行做业务逻辑安全评估" |
